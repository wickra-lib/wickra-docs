...
[LinRegSlope](Indicator-LinRegSlope) (which returns the continuous slope).
...
[LinRegSlope](Indicator-LinRegSlope), keeping `TrendLabel` as the pure sign.
...
- **`0` is rare on real data.** A perfectly flat regression is unusual; expect
  ...
  [LinRegSlope](Indicator-LinRegSlope) yourself.
...
- [Indicator-LinRegSlope](Indicator-LinRegSlope) — the continuous slope.
- [Indicator-LinRegAngle](Indicator-LinRegAngle) — slope as an angle.
