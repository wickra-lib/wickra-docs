...
[`HilbertDominantCycle`](Indicator-HilbertDominantCycle)) to find the dominant
...
what [`HtTrendMode`](Indicator-HtTrendMode) keys on. Use the phase to time
...
- **Treating the phase as price-predictive in a trend.** When the market is
  ...
  [`HtTrendMode`](Indicator-HtTrendMode) and only act on the phase in cycle mode.
...
- [Indicator-HtPhasor](Indicator-HtPhasor) — the quadrature pair the phase is derived from.
- [Indicator-HtTrendMode](Indicator-HtTrendMode) — trend/cycle classification built on the phase.
