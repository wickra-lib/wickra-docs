...
[`PlusDi`](Indicator-PlusDi). The shared `directional_movement` helper lives in
...
[`PlusDi`](Indicator-PlusDi) and combined with [`MinusDi`](Indicator-MinusDi)
...
- **Comparing magnitudes across instruments.** `+DM` is in raw price units, so
  ...
  [`PlusDi`](Indicator-PlusDi) (normalised by true range, `0–100`) for
...
- [Indicator-MinusDm](Indicator-MinusDm) — the bearish counterpart.
- [Indicator-PlusDi](Indicator-PlusDi) — `+DM` normalised by true range.
