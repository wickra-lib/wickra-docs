...
[`Sma`](Indicator-Sma), which sums every value, the median depends only on the
...
- [Indicator-Sma](Indicator-Sma) — the sum-based mean the median MA is robust against.
- [Indicator-MedianPrice](Indicator-MedianPrice) — `(high + low) / 2`, a different "median".
- [Indicator-MedianAbsoluteDeviation](Indicator-MedianAbsoluteDeviation) — robust dispersion companion.
