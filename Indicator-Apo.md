...
line; use [MacdIndicator](Indicator-MacdIndicator) when you also need the
...
3. **Magnitude = momentum strength**, but in *price units* — so APO values
   ...
   [Ppo](Indicator-Ppo), the percentage variant, for that).
...
- **Comparing APO across symbols.** It is denominated in price units; a
  ...
  [Ppo](Indicator-Ppo) (percentage price oscillator) for cross-instrument
  comparison.
- **Expecting a signal line.** APO has none — pair it with your own EMA, or
  use [MacdIndicator](Indicator-MacdIndicator) which bundles the signal and
  histogram.
...
- [MacdIndicator](Indicator-MacdIndicator) — APO plus a signal line and
  histogram.
- [Ppo](Indicator-Ppo) — the percentage-normalised version.
- [Ema](Indicator-Ema) — the building block.
