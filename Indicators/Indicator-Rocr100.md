...
This is [`Rocr`](/Indicators/Indicator-Rocr) scaled by 100, so the flat line moves from `1`
...
`Rocr100` is identical in shape to [`Rocr`](/Indicators/Indicator-Rocr) but reads on a
...
- **Reading it as a percentage change.** `ROCR100` is the *ratio* ×100, not the
  *change* ×100. A +10% move reads `110`, not `10`; the percentage change is
  `ROCR100 − 100` (here `10`), which is [`Roc`](/Indicators/Indicator-Roc).
...
- [Indicator-Rocr](/Indicators/Indicator-Rocr) — the same ratio centred on `1`.
- [Indicator-Rocp](/Indicators/Indicator-Rocp) — the fractional form.
- [Indicator-Roc](/Indicators/Indicator-Roc) — the percentage-change form (`ROCR100 − 100`).
