...
> that [Value Area](Indicator-ValueArea) reduces to POC / VAH / VAL.
...
one bin) — the same bucketing convention as [`ValueArea`](Indicator-ValueArea).
...
volume are the Value Area. Use [`ValueArea`](Indicator-ValueArea) when you only
...
- **Confusing it with TPO.** Volume Profile weights by traded volume;
  [`TpoProfile`](Indicator-TpoProfile) counts time at price and ignores volume.
...
- [Indicator-ValueArea](Indicator-ValueArea) — POC / VAH / VAL from this distribution.
- [Indicator-TpoProfile](Indicator-TpoProfile) — the time-based (volume-agnostic) profile.
