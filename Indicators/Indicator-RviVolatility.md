...
> the shorter [`RVI`](/Indicators/Indicator-Rvi) name.
...
falls back to `50`, the same undefined-RS convention as [Rsi](/Indicators/Indicator-Rsi).
...
to `float | None` (`RVIVolatility.update`) / an `array.array('d')` with
...
- **Confusing it with the Relative Vigor Index.** That is a different Dorsey
  indicator (momentum, not volatility) — see [Rvi](/Indicators/Indicator-Rvi). They share
...
- **Reading it as a volatility *level*.** It is a `0–100` direction ratio;
  ...
  [HistoricalVolatility](/Indicators/Indicator-HistoricalVolatility) or the OHLC
  estimators.
...
- [Rvi](/Indicators/Indicator-Rvi) — the *other* Dorsey RVI: Relative Vigor Index, a
  momentum indicator in the Momentum family.
- [Rsi](/Indicators/Indicator-Rsi) — the same Wilder ratio applied to gain/loss instead
  of stddev.
- [StdDev](/Indicators/Indicator-StdDev) — the per-bar building block.
- [HistoricalVolatility](/Indicators/Indicator-HistoricalVolatility) — annualised level
