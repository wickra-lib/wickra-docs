...
cycle, computes the [dominant-cycle phase](Indicator-HtDcPhase), and derives the
...
(moving-average crossovers, breakouts, [`SarExt`](Indicator-SarExt)) tend to
work, while mean-reversion oscillators give false signals; in cycle mode (`0`) the
reverse holds — oscillators like [`Rsi`](Indicator-Rsi) and cycle tools shine and
...
- **Trading the flag directly.** A flip from `0` to `1` is a regime change, not a
  ...
  average or [`HtDcPhase`](Indicator-HtDcPhase)) for direction.
...
- [Indicator-HtDcPhase](Indicator-HtDcPhase) — the dominant-cycle phase it is built on.
- [Indicator-HtPhasor](Indicator-HtPhasor) — the underlying quadrature pair.
- [Indicator-HilbertDominantCycle](Indicator-HilbertDominantCycle) — the dominant cycle period.
