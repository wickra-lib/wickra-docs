...
> `(1 + v)·EMA − v·EMA(EMA)`, the building block of the [`T3`](Indicator-T3)
...
- `v = 1` → `GD = 2·EMA − EMA(EMA)` = the standard [`Dema`](Indicator-Dema).
...
factor) is exactly how [`T3`](Indicator-T3) is built.
...
[`Dema`](Indicator-Dema). For `period = 5` that is `9`; the first non-`None`
...
choice if you intend to study the [`T3`](Indicator-T3) family, since T3 is just
...
- [Indicator-Dema](Indicator-Dema) — GD at `v = 1`.
- [Indicator-Ema](Indicator-Ema) — GD at `v = 0`, the building block.
- [Indicator-T3](Indicator-T3) — GD applied three times with the binomial volume factor.
