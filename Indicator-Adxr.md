...
> current [ADX](Indicator-Adx) and the ADX from `period − 1` bars ago. A more
...
[Adx](Indicator-Adx). Because the older ADX is `period − 1` bars stale, ADXR
...
- **Reading direction from it.** ADXR (like ADX) measures trend *strength*,
  not direction — pair it with `+DI`/`−DI` (see [Adx](Indicator-Adx)) for
  direction.
...
- [Vortex](Indicator-Vortex) — alternative directional-trend gauge.
- [Rwi](Indicator-Rwi) — trend-vs-random-walk strength.
