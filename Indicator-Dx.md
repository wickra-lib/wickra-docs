...
where [`+DI`](Indicator-PlusDi) and [`−DI`](Indicator-MinusDi) are derived from
Wilder-smoothed `+DM`, `−DM` and true range over `period` bars. `DX` is the
*un-smoothed* directional index; [`Adx`](Indicator-Adx) is its Wilder average.
...
which is exactly why Wilder averaged it into [`Adx`](Indicator-Adx). Use `DX`
...
Pair `DX`/`Adx` (strength) with [`PlusDi`](Indicator-PlusDi) /
[`MinusDi`](Indicator-MinusDi) (direction) for the complete picture.
...
- **Expecting smooth output.** `DX` whipsaws bar to bar; that noise is by design.
  Use [`Adx`](Indicator-Adx) if you need a stable line.
...
- [Indicator-Adx](Indicator-Adx) — the Wilder average of `DX`.
- [Indicator-PlusDi](Indicator-PlusDi) / [Indicator-MinusDi](Indicator-MinusDi) — the directional legs.
