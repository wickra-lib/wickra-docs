...
where `+DM` is the [plus directional movement](Indicator-PlusDm) and `TR` is the
...
[`MinusDi`](Indicator-MinusDi): `+DI > −DI` marks an up-trending regime, and the
two lines crossing is a classic trend-change signal. The spread between them,
normalised, is [`Dx`](Indicator-Dx); the Wilder average of `DX` is
[`Adx`](Indicator-Adx), which measures trend *strength* regardless of direction.
...
- [Indicator-MinusDi](Indicator-MinusDi) — the bearish counterpart.
- [Indicator-PlusDm](Indicator-PlusDm) — the un-normalised numerator.
