...
Where [`LinearRegression`](/Indicators/Indicator-LinearRegression) evaluates the fit at the
...
`TSF` and [`LinearRegression`](/Indicators/Indicator-LinearRegression) differ only by one bar
...
- [Indicator-LinRegIntercept](/Indicators/Indicator-LinRegIntercept) — the fit at the window start.
- [Indicator-LinRegSlope](/Indicators/Indicator-LinRegSlope) — the per-bar slope.
