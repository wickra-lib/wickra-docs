...
The [Tsf](Indicator-Tsf) term is the OLS line fitted over the last `period`
bars, evaluated **one bar past** the window (`a + b·period`). Where
[Cfo](Indicator-Cfo) measures the same percentage gap against the regression
...
`warmup_period()` returns `period`. The oscillator wraps a [Tsf](Indicator-Tsf)
...
2. **Trend bias vs CFO.** Because the forecast is one bar ahead, a clean
   ...
   fixed zero, or pair it with [Cfo](Indicator-Cfo) for the current-bar gap.
...
- **Confusing it with the forecast.** The oscillator is the *deviation* from
  the [Tsf](Indicator-Tsf) value, not the forecast itself — use `Tsf`
...
- [Cfo](Indicator-Cfo) — the current-bar regression gap; this oscillator's
  one-bar-ahead companion.
- [Tsf](Indicator-Tsf) — the one-bar-ahead forecast this oscillator deviates
  from.
- [LinearRegression](Indicator-LinearRegression) — the current-bar endpoint.
