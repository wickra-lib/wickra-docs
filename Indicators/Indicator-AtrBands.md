...
Unlike [Keltner](/Indicators/Indicator-Keltner) or [StarcBands](/Indicators/Indicator-StarcBands),
...
`warmup_period()` delegates to the inner [`Atr`](/Indicators/Indicator-Atr), which uses
Wilder's seeding: the first [`Atr`](/Indicators/Indicator-Atr) value is the simple
...
[AtrTrailingStop](/Indicators/Indicator-AtrTrailingStop) instead when you want a stop
that only ratchets in your favour.

## Common pitfalls

- **Reading the midline as a trend filter.** It is the raw close, not a
  moving average — it carries no smoothing. Use [StarcBands](/Indicators/Indicator-StarcBands)
  or [Keltner](/Indicators/Indicator-Keltner) if you want a smoothed centerline.
...
- [Atr](/Indicators/Indicator-Atr) — the volatility scale used.
- [AtrTrailingStop](/Indicators/Indicator-AtrTrailingStop) — directional one-sided
  variant that ratchets only in favour of an open position.
- [SuperTrend](/Indicators/Indicator-SuperTrend) — band that flips sides on a close
