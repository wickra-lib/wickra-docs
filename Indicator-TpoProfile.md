...
[`VolumeProfile`](Indicator-VolumeProfile), which distributes *volume*. See
...
Read alongside [`VolumeProfile`](Indicator-VolumeProfile): when the time-POC and
...
- **Treating counts as volume.** A tall TPO bin means *many periods* traded
  there, not high volume. For volume use [`VolumeProfile`](Indicator-VolumeProfile).
...
- [Indicator-VolumeProfile](Indicator-VolumeProfile) — the volume-weighted profile.
- [Indicator-ValueArea](Indicator-ValueArea) — POC / VAH / VAL summary levels.
