...
[`MinusDi`](/Indicators/Indicator-MinusDi). The shared `directional_movement` helper lives in
...
`MinusDm::new(period).warmup_period() == period + 1` (the `accessors_report_config`
...
[`MinusDi`](/Indicators/Indicator-MinusDi) and combined with [`PlusDi`](/Indicators/Indicator-PlusDi)
...
- **Comparing magnitudes across instruments.** `−DM` is in raw price units; use
  [`MinusDi`](/Indicators/Indicator-MinusDi) (`0–100`) for cross-instrument comparison.
...
- [Indicator-PlusDm](/Indicators/Indicator-PlusDm) — the bullish counterpart.
- [Indicator-MinusDi](/Indicators/Indicator-MinusDi) — `−DM` normalised by true range.
