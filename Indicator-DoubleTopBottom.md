...
- **Python.** `update((o,h,l,c,v,ts))` → `float` (`0.0` until a pattern, never
  `None`); `batch(open, high, low, close)` → 1-D `ndarray`.
...
- [TripleTopBottom](Indicator-TripleTopBottom) — the three-extreme variant.
- [HeadAndShoulders](Indicator-HeadAndShoulders) — asymmetric three-peak reversal.
