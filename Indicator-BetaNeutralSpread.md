...
[PairSpreadZScore](Indicator-PairSpreadZScore) standardises this residual into a
z-score and [Cointegration](Indicator-Cointegration) bundles it with an ADF
...
with [PairSpreadZScore](Indicator-PairSpreadZScore) when ranking signals across
pairs.
...
- **Rolling β drifts.** The hedge ratio is refit every bar, so a structural
  ...
  the relationship is stable with [Cointegration](Indicator-Cointegration).
...
- [Indicator-PairSpreadZScore](Indicator-PairSpreadZScore) — standardised entry/exit timing.
- [Indicator-Cointegration](Indicator-Cointegration) — hedge ratio + ADF stationarity test.
