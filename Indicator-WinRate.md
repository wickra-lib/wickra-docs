...
[Expectancy](Indicator-Expectancy), Kelly sizing, and confidence filters.
...
[Expectancy](Indicator-Expectancy) (per-trade edge in risk units) and
[ProfitFactor](Indicator-ProfitFactor) / [KellyCriterion](Indicator-KellyCriterion)
for a full picture.
...
- **Win rate is not edge.** A 70% win rate with a 1:5 reward:risk can be a losing
  ...
  [Expectancy](Indicator-Expectancy).
...
- [Indicator-Expectancy](Indicator-Expectancy) — per-trade edge in risk units.
- [Indicator-ProfitFactor](Indicator-ProfitFactor) — gross profit / gross loss.
