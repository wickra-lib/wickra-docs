...
[Ppo](Indicator-Ppo) itself emits only the percentage line; this indicator
...
asset, unlike the price-unit [MacdHistogram](Indicator-MacdHistogram).
...
Python maps this to `float | None` / a `float64` `np.ndarray` with `NaN`
...
For the PPO line on its own, use [Ppo](Indicator-Ppo). For the price-unit
version, see [MacdHistogram](Indicator-MacdHistogram).
...
- [Ppo](Indicator-Ppo) — the PPO line this histogram is built on.
- [MacdHistogram](Indicator-MacdHistogram) — the price-unit counterpart.
- [Apo](Indicator-Apo) — the absolute (price-unit) price oscillator.
