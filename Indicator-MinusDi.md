...
where `−DM` is the [minus directional movement](Indicator-MinusDm) and `TR` is
...
`MinusDi::new(period).warmup_period() == period` (the `accessors_report_config`
...
[`PlusDi`](Indicator-PlusDi): `−DI > +DI` marks a down-trending regime, and a
`−DI`/`+DI` cross is a trend-change signal. The normalised spread between the two
is [`Dx`](Indicator-Dx); its Wilder average is [`Adx`](Indicator-Adx), which
...
- [Indicator-PlusDi](Indicator-PlusDi) — the bullish counterpart.
- [Indicator-MinusDm](Indicator-MinusDm) — the un-normalised numerator.
- [Indicator-Dx](Indicator-Dx) / [Indicator-Adx](Indicator-Adx) — the spread and its smoothed average.
