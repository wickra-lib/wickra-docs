...
- **Adaptive period estimator.** The standard use is to feed half
  ...
  (RSI, Stoch, CCI). See [AdaptiveCycle](Indicator-AdaptiveCycle)
...
- [AdaptiveCycle](Indicator-AdaptiveCycle) — half-period wrapper for
  adaptive oscillators.
- [Mama](Indicator-Mama) — uses the same phase machinery for
  adaptive smoothing.
- [SineWave](Indicator-SineWave) — sine + leadsine from the same
  phase estimate.
