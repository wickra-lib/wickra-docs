...
over the anchored window. Where [`Rsi`](Indicator-Rsi) applies Wilder's
...
reads the neutral `50` — identical to the convention used by [`Rsi`](Indicator-Rsi).
...
| Method | Description |
|--------|-------------|
| `set_anchor()` | Marks a re-anchor; the **next** `update` clears the running sums and previous close before folding in its own bar, starting a fresh anchored window. Mirrors [`AnchoredVwap`](Indicator-AnchoredVwap)'s "click to anchor" workflow. |
...
steadier than a rolling [`Rsi`](Indicator-Rsi) deep into a trend (one more bar
barely moves a long-running average), which makes divergence against the anchored
leg easy to read. Use the rolling [`Rsi`](Indicator-Rsi) when you want a
...
- **Forgetting the re-seed after `set_anchor()`.** Because RSI needs a price
  ...
  [`AnchoredVwap`](Indicator-AnchoredVwap), which emits immediately).
...
- [Indicator-Rsi](Indicator-Rsi) — the fixed-period Wilder RSI.
- [Indicator-AnchoredVwap](Indicator-AnchoredVwap) — the volume-weighted anchored sibling.
