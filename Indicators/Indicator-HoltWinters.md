...
A plain [`Ema`](/Indicators/Indicator-Ema) carries only a level, so it lags a sustained
...
would bias the envelope. Compare against [`Dema`](/Indicators/Indicator-Dema) /
[`Tema`](/Indicators/Indicator-Tema): Holt achieves lag reduction through an explicit trend
...
- [Indicator-Ema](/Indicators/Indicator-Ema) — single exponential smoothing (level only).
- [Indicator-Dema](/Indicators/Indicator-Dema) — EMA-cascade lag reduction.
- [Indicator-GeneralizedDema](/Indicators/Indicator-GeneralizedDema) — tunable EMA-cascade lag reduction.
