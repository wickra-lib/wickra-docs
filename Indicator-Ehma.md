...
> recursive, smoother variant of the [`Hma`](Indicator-Hma).
...
Versus the WMA-based [`Hma`](Indicator-Hma): the EMA components have infinite
...
- **NaN / infinity inputs.** Inherited from the inner [`Ema`](Indicator-Ema):
...
`Ehma` is the recursive sibling of [`Hma`](Indicator-Hma): the same
...
- [Indicator-Hma](Indicator-Hma) — the canonical WMA-based Hull average.
- [Indicator-Ema](Indicator-Ema) — the building block.
- [Indicator-Tema](Indicator-Tema) — triple-EMA lag reduction without the Hull diff.
