...
[`LinRegSlope`](Indicator-LinRegSlope) — positive while price trends up,
...
scales, which the raw [`LinRegSlope`](Indicator-LinRegSlope) cannot do.
...
- [Indicator-LinRegSlope](Indicator-LinRegSlope) — the same fit's slope,
  in raw price-per-bar units.
- [Indicator-LinearRegression](Indicator-LinearRegression) — the
